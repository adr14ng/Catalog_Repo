...
//------------------------------------------------------------------------------
add_filter('pre_get_posts', 'rme_remove_sub');
add_action('admin_head-post.php', 'rme_remove_sub');
add_action('admin_head-post-new.php', 'rme_remove_sub');
add_filter('admin_menu', 'rme_remove_sub', 999);
function rme_remove_sub() {

	if( current_user_can('dp_map_editor') ){
		$id = get_the_ID();
		global $submenu;
		  unset($submenu['edit.php?post_type=transfer_plans'][10]);


		global $pagenow;
		if($pagenow == 'edit.php' && $_GET['post_type'] == 'transfer_plans'){
			echo '<style type="text/css">.page-title-action{display: none;}</style>';
		}

		remove_menu_page('edit.php?post_type=staract', 'edit-tags.php?taxonomy=category&amp;post_type=staract');
		remove_menu_page('edit.php');
		remove_menu_page('edit-tags.php?taxonomy=department_shortname&amp;post_type=courses');
		remove_menu_page('edit-tags.php?taxonomy=post_tag&amp;post_type=programs');
		remove_menu_page('edit-tags.php?taxonomy=category&amp;post_type=departments');
		remove_menu_page('upload.php');
		remove_menu_page('edit.php?post_type=groups');
		remove_menu_page('edit-tags.php?taxonomy=post_tag&amp;post_type=policies');
		remove_menu_page('edit.php?post_type=plans');
		remove_menu_page('proposals');
		remove_menu_page('edit.php?post_type=page');
		remove_menu_page('review');
...
add_filter('pre_get_posts', 'rme_remove_post_sub' );
function rme_remove_post_sub( $query ) {

    if ( is_user_logged_in() and current_user_can( 'dp_map_editor' ) )  {
...
function rme_remove_toolbar($wp_admin_bar) {
...
add_action('admin_bar_menu', 'rme_remove_toolbar', 999);

add_filter( 'get_sample_permalink_html', 'rme_remove_permalink' );
function rme_remove_permalink( $return ) {
	if( current_user_can('dp_map_editor') ){
    $return = '';
	}
    return $return;
...
function the_breadcrumb() {
    ...
    if (!is_home()) {
		...
		//Get college
		$donotuse = $col->description;
		if (strpos($donotuse, "Used to sort classes -- Not departments!") === FALSE) {
...
function csun_title_text() {
	...
	elseif(is_singular('staract')) :
		...
		echo $title.' ('.$aca_year.') - STAR Act Degree Road Map';

	elseif(is_singular('transfer_plans')) :
		$title = get_the_title();
		$id = get_the_ID();
		$years = get_the_terms( $id, 'aca_year');
		...
		echo $title.' ('.$aca_year.') - Transfer Degree Road Map';
	...
	elseif(is_singular('plans')) :
		...
		echo $title.' ('.$aca_year.') - Degree Road Map';
	...
	elseif(is_tax('aca_year')) :
		...
		if(!isset($type) || $type == ''){
			...
			if ( false !== strpos($url, 'star-act') )
				$type = 'STAR Act Degree Road Maps';
			elseif ( true == strpos($url, 'transfer-road-map') )
				$type = 'Transfer Degree Road Maps';
			else
				$type = 'Degree Road Maps';
		}
...
function csun_select_template( $template ){
	...
	if(isset($wp_query->query_vars['department_shortname']) && isset($wp_query->query_vars['post_type'])
		&& ($wp_query->query_vars['post_type'] === 'plans' || $wp_query->query_vars['post_type'] === 'staract' || $wp_query->query_vars['post_type'] === 'transfer_plans'))
	{
 ...
 */
 function add_ge_links( $content )
 {
 	...
 	'(?:GE '.									//Typical GE course start
 		...
 		'([A-Za-z0-9 ]*)'.						//The GE category
 	')|'.										//or
 	'(Title (?: Five|5|V)(?: requirement| American History Institutions and Ideals| US Constitution and California State and Local Government)?)'.	//Title 5 type
 	'/i';										//Don't worry about case

 	if(is_singular('plans') || is_singular('staract') || is_singular('transfer_plans')) :
 		$content = preg_replace_callback(
 			...
 			function ($matches) {
 				...
 				if(!empty($matches[2])) {	//typical ge section
 					...
 					elseif(stripos($matches[2], 'American History Institutions and Ideals') !== false)
 					{
 						$url .= 't1';
 						$name = 't5';
 					}elseif(stripos($matches[2], 'US Constitution and California State and Local Government') !== false)
 					{
 						$url .= 't2';
 						$name = 't5';
 					}
 				...
 				elseif(!empty($matches[3])) {	//title 5 section
 					if(stripos($matches[3], 'American History Institutions and Ideals') !== false){
 						$url .= 't1';
 						$name = 't5';
 					} elseif(stripos($matches[3], 'US Constitution and California State and Local Government') !== false){
 						$url .= 't2';
 						$name = 't5';
 					}
 					/* elseif(){
...
/**
*
* This function hooks into the advanced custom field in Departments--> Field_Instructions
* and will display
* @return $String echo: About the Department
* or
*                 echo: About the Program
*/

function acf_about_dep_instructions( $about_dep )
{
		/*$dep_post_cat = get_category();
			$cat_slug = $dep_post_cat[0]->cat_name;


			if ($cat_slug === "dep"){
				$cat_slug_msg = 'About the Department';
				}
			elseif ($cat_slug === "pro"){
				$cat_slug_msg = 'About the Program';
				}
			else;
  $about_dep['instructions'] = array(
     'custom' => $cat_slug_msg);
		*/
		$about_dep['Instructions'] = array(
...
add_filter('field_572b8d731dd84','acf_about_dep_instructions');
